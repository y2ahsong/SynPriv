...
class Tabula:
    ...
    def save(self, path: str):
        ...
        with open(path + "/config.json", "w") as f:
            ...
            attributes.pop("label_encoder_list", None)
            attributes["num_cols"] = self.num_cols
            attributes["start_col"] = self.conditional_col
            
            # if isinstance(attributes["config"], np.ndarray):
            #     attributes["config"] = list(attributes["config"])
            ...
            attributes["columns"] = self.columns
            
            json.dump(attributes, f)

        torch.save(self.model.state_dict(), path + "/model.pt")
        if hasattr(self, "label_encoder_list"):
            import pickle
            with open(path + "/label_encoders.pkl", "wb") as f:
                pickle.dump(self.label_encoder_list, f)
