from src.clava.pipeline_utils import *
import numpy as np
import os
import pandas as pd
import json
...
import pickle
import time
from src.clava.tab_ddpm.utils import *
from src.clava.pipeline_modules import *
from sdv.metadata import MultiTableMetadata
from src.clava.gen_multi_report import gen_multi_report
...
def clava_eval(tables, save_dir, configs, relation_order, synthetic_tables=None):
    ...
    for table_name, val in tables.items():
        ...
        if table_name == 'meta':
            metadata.update_column(table_name, 'meta_id', sdtype='id')
            metadata.set_primary_key(table_name=table_name, column_name='meta_id')
        elif table_name == 'flow':
            metadata.update_column(table_name, 'flow_id', sdtype='id')
            metadata.set_primary_key(table_name=table_name, column_name='flow_id')
            metadata.update_column(table_name, 'meta_id', sdtype='id')
        ...
        # if table_name == 'meta':
        #     metadata.set_primary_key(table_name=table_name, column_name='meta_id', sdtype='id')
        # elif table_name == 'flow':
        #     metadata.set_primary_key(table_name=table_name, column_name='flow_id', sdtype='id')

    for parent, child in relation_order:
        if parent is not None:
            metadata.add_relationship(
                parent_table_name='meta',
                child_table_name='flow',
                parent_primary_key='meta_id',
                child_foreign_key='meta_id'
            )
